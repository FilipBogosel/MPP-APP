...
export function useServiceDetailController(recordId?: string): UseServiceDetailControllerResult {
  ...
  const initialData = useMemo<ServiceRecordFormData | null>(() => (selectedRecord ? mapRecordToFormData(selectedRecord) : null), [selectedRecord]);
  ...
  const hasChanges = useMemo(() => initialData !== null && formData !== null && JSON.stringify(formData) !== JSON.stringify(initialData), [formData, initialData]);

  const onFieldChange = (event: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
  ...
  const onUpdate = () => {
    ...
    if (!validationResult.success) {
      setValidationErrors(mapValidationErrors(validationResult.error.flatten().fieldErrors));
      return;
    }

    setValidationErrors({});
    updateRecord(recordId, buildUpdatedRecord({ cars, formData, selectedRecord }));
