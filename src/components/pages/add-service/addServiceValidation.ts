...
// Service type validation schema with strict enum values
const addServiceValidationSchema = z
  .object({
    ...
    nextKilometers: z
      .number({
        ...
        invalid_type_error:
          "Next recommended service must be a positive number.",
      })
  ...
  .strict()
  .refine((values) => values.nextKilometers > values.kilometers, {
    path: ["nextKilometers"],
